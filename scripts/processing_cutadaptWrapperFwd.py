...
if os.path.getsize(snakemake.input[0]) == 0:
    ...
    cmd = ["cutadapt", "-g" , "%(fwd_primer)s...%(rv_rev_primer)s" % snakemake.config, "--discard-untrimmed", "--cores",  snakemake.threads, "--error-rate", snakemake.config["primerErr"], "-o", snakemake.output[0], snakemake.input[0]]
